...
export async function POST(req: NextRequest) {
  ...
  if (user.role === 'doctor') {
    ...
    if (!doctor) return NextResponse.json({ error: 'Doctor not found' }, { status: 404 });
    db.prepare(`
      INSERT INTO custom_orders (doctor_id, patient_id, created_by_role, category, description, photos, photos_affected, photos_unaffected, body_parts, material_id, payment_target)
      VALUES (?, ?, 'doctor', ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(doctor.id, body.patient_id || null, body.category || null, body.description.trim(), photosStr, photosAffectedStr, photosUnaffectedStr, body.body_parts || null, body.material_id || null, paymentTarget);
  } else if (user.role === 'po_specialist') {
    const specialist = db.prepare('SELECT id FROM po_specialists WHERE user_id = ?').get(user.id) as { id: number } | undefined;
    if (!specialist) return NextResponse.json({ error: 'Specialist not found' }, { status: 404 });
    db.prepare(`
