...
// PATCH: allows doctor or patient to update fulfillment_status on their own
// custom orders — mirrors app/api/orders/[id]/route.ts for standard orders.
...
export async function PATCH(
  ...
  if (user.role === 'doctor') {
    ...
    const order = db.prepare(
      'SELECT id FROM custom_orders WHERE id = ? AND doctor_id = ?'
    ).get(id, doctor.id);
    if (!order) return NextResponse.json({ error: 'Order not found or access denied.' }, { status: 404 });

    db.prepare('UPDATE custom_orders SET fulfillment_status = ? WHERE id = ?').run(body.fulfillment_status, id);
  ...
  if (user.role === 'patient') {
    ...
    const order = db.prepare('SELECT id FROM custom_orders WHERE id = ? AND patient_id = ?').get(id, patient.id);
    if (!order) return NextResponse.json({ error: 'Order not found or access denied.' }, { status: 404 });
