...
export default function ReceptionistProfilePage() {
  return (
    <Suspense fallback={<div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>Loading...</div>}>
      <ReceptionistProfilePageInner />
    </Suspense>
  );
}

function ReceptionistProfilePageInner() {
  ...
  const [editForm, setEditForm] = useState<Partial<ReceptionistProfile>>({});
  ...
  async function loadProfile() {
    try {
      const r = await fetch('/api/receptionist/profile');
    ...
    try {
      ...
      if (!res.ok) { setEditError(data.error || 'Failed to update profile.'); }
        ...
        if (missing.length > 0) {
          ...
          setEditing(false);
          if (requiredMode) router.replace('/dashboard/receptionist/profile');
        }
        loadProfile();
      }
      ...
      <div className="dash-page-header" style={{ marginBottom: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            ...
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '2px 12px', borderRadius: 20, background: '#d1fae5', color: '#065f46', fontSize: '0.8rem', fontWeight: 600, marginTop: 4 }}>
              Receptionist
            </div>
          </div>
        </div>
