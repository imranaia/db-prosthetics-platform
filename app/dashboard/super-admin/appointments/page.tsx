...
import { CalendarDays, Home, Building2 } from 'lucide-react';
      ...
      <div className="skeu-card" style={{ padding: 0, overflow: 'hidden' }}>
        <div className="table-scroll">
          <table className="dash-table">
            <thead><tr><th>Patient</th><th>Type</th><th>Status</th><th>Scheduled</th><th>Notes</th><th>Actions</th></tr></thead>
            <tbody>
              {visible.length === 0 ? (
                <tr><td colSpan={6} style={{ padding: '40px', textAlign: 'center', color: 'var(--text-muted)' }}>No appointments in this category.</td></tr>
              ) : visible.map(a => {
                const ss = S_STYLE[a.status] || { bg: '#f3f4f6', color: '#6b7280' };
                return (
                  <tr key={a.id}>
                    <td style={{ fontWeight: 600, color: 'var(--text-head)' }}>{a.patient_name}</td>
                    <td>
