...
export default function LoginPage() {
  const [mode,     setMode]     = useState<'email' | 'patient_id'>('email');
  ...
  return (
    <div className="auth-bg">
      <div className="auth-card">
        ...
        {/* Login mode toggle */}
        <div style={{ display: 'flex', gap: '6px', marginBottom: '22px', background: 'var(--bg-base)', padding: '4px', borderRadius: '10px' }}>
          <button
            type="button"
            ...
            style={{
              ...
              background: mode === 'email' ? 'var(--bg-white)' : 'transparent',
              color: mode === 'email' ? 'var(--primary)' : 'var(--text-muted)',
              boxShadow: mode === 'email' ? '2px 2px 6px var(--shadow-dark), -2px -2px 6px var(--shadow-light)' : 'none',
            }}
          >
            Email
          </button>
          <button
            type="button"
            onClick={() => { setMode('patient_id'); setError(''); }}
            style={{
              flex: 1, padding: '9px', borderRadius: '7px', border: 'none', cursor: 'pointer',
              fontSize: '0.82rem', fontWeight: 600,
              background: mode === 'patient_id' ? 'var(--bg-white)' : 'transparent',
              color: mode === 'patient_id' ? 'var(--primary)' : 'var(--text-muted)',
              boxShadow: mode === 'patient_id' ? '2px 2px 6px var(--shadow-dark), -2px -2px 6px var(--shadow-light)' : 'none',
            }}
          >
            Patient ID
          </button>
        ...
        <form onSubmit={handleSubmit} noValidate>
          {mode === 'email' ? (
            ...
            <>
              <div style={{ marginBottom: '20px' }}>
                <label className="skeu-label" htmlFor="patient_id">Patient ID</label>
                <input
                  id="patient_id" type="text" className="skeu-input"
                  placeholder="DBP-00001"
